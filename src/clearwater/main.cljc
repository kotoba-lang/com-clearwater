(ns clearwater.main
  "Kotodama WASM entrypoint for the Clearwater clean-room actor (L4) — Clojure port.
  ...
  facts (`clearwater.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "clearwater")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "clearwat_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}                   :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "clearwat_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"clearwater.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "clearwater-compat" :tier tier :entities entities} 200])
...
(defn start! [] :clearwater-compat/ready)
